...
// ─── BRAND PALETTE (extracted from JPC logo) ────────────────────────────────
const B = {
  brown: "#4A2C0A",   // "JPC TREE CARE LLC" text — primary brand color
  brownLight: "#6B3F10",   // hover/lighter brown
  brownDark: "#2E1A06",   // deep shadow brown
  greenDark: "#1B5E2F",   // inner leaves — deep forest
  greenMid: "#2E7D32",   // middle leaves — rich green
  greenLight: "#7CB342",   // outer leaves — lime/fresh
  greenPale: "#F1F8E9",   // tint background
  greenAccent: "#8BC34A",   // highlight / accent lime
  offWhite: "#FAFAF7",   // warm off-white bg
  lightGray: "#F4F0EC",   // warm light gray (brown-tinted)
  text: "#1C0F05",   // near-black warm
  textMuted: "#7A5C42",   // muted brown-gray
  textLight: "#B08060",   // light warm text
};
...
const T = {
  en: {
    ...
    services: {
      ...
      items: [
        { title: "Pruning & Trimming", desc: "Shape and strengthen your trees for health, safety, and curb appeal.", tag: "Most Popular" },
        { title: "Tree Removal", desc: "Safe, efficient removal of any tree — no job too large or dangerous.", tag: "Emergency Available" },
        { title: "Lot Clearing", desc: "Full land clearing for construction, landscaping, or reclaiming your space.", tag: "Free Quote" },
      ],
    },
    why: {
  ...
  es: {
    ...
    services: {
      ...
      items: [
        { title: "Poda y Recorte", desc: "Dale forma y salud a tus árboles para mayor seguridad y belleza.", tag: "Más Popular" },
        { title: "Remoción de Árboles", desc: "Remoción segura y eficiente de cualquier árbol.", tag: "Emergencias Disponibles" },
        { title: "Limpieza de Terrenos", desc: "Limpieza total para construcción, paisajismo o recuperación de terrenos.", tag: "Cotización Gratis" },
      ],
    },
    why: {
      title: "¿Por Qué JPC?",
...
// ─── MEDIA PLACEHOLDER ───────────────────────────────────────────────────────
function MediaSlot({ type = "photo", label, hint, style = {}, rounded = 16 }) {
  return (
    <div style={{
      position: "relative", overflow: "hidden", borderRadius: rounded,
      background: `linear-gradient(145deg, ${B.brownDark} 0%, #0d1a0b 100%)`,
      border: `1.5px dashed rgba(139,195,74,0.3)`,
      display: "flex", flexDirection: "column", alignItems: "center",
      justifyContent: "center", gap: 10, padding: "24px 20px", textAlign: "center",
      ...style,
    }}>
      <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(46,125,50,0.3)", border: "1px solid rgba(139,195,74,0.3)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        {type === "video" ? <Video size={20} color={B.greenAccent} /> : <Image size={20} color={B.greenAccent} />}
      </div>
      <div>
        <div style={{ color: B.greenAccent, fontWeight: 700, fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 4 }}>
          {type === "video" ? "📹 VIDEO" : "📷 FOTO"}
        </div>
        <div style={{ color: "rgba(255,255,255,0.9)", fontWeight: 600, fontSize: 13, marginBottom: 6 }}>{label}</div>
        <div style={{ color: "rgba(139,195,74,0.7)", fontSize: 11, lineHeight: 1.5, fontFamily: "monospace", maxWidth: 280 }}>{hint}</div>
      </div>
      {type === "video" && (
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
          <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center" }}>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <img
        ...
        style={{ height, width: "auto", objectFit: "contain", filter: darkBg ? "brightness(0) invert(1)" : "none" }}
      />
    </div>
  );
}
    ...
    <div style={{ fontFamily: "'Libre Baskerville', Georgia, serif", color: B.text, overflowX: "hidden", background: B.offWhite }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn-cta:hover {
          background: ${B.brownLight};
          transform: translateY(-2px);
          box-shadow: 0 8px 28px rgba(74,44,10,0.38);
        }
      ...
      <header style={{
        ...
        borderBottom: scrolled ? `1px solid rgba(74,44,10,0.08)` : "none",
        boxShadow: scrolled ? "0 2px 24px rgba(74,44,10,0.07)" : "none",
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 24px", height: 72, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="desk-only" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => scrollTo("services")} style={{ background: "none", border: "none", cursor: "pointer", color: scrolled ? B.textMuted : "rgba(255,255,255,0.75)", fontSize: 14, fontFamily: "'Outfit',sans-serif", fontWeight: 500, padding: "6px 10px" }}>
              {t.nav.estimate === "Free Estimate" ? "Services" : "Servicios"}
            </button>
            <button onClick={() => scrollTo("testimonials")} style={{ background: "none", border: "none", cursor: "pointer", color: scrolled ? B.textMuted : "rgba(255,255,255,0.75)", fontSize: 14, fontFamily: "'Outfit',sans-serif", fontWeight: 500, padding: "6px 10px" }}>
              {lang === "en" ? "Reviews" : "Reseñas"}
            </button>
            <button onClick={() => scrollTo("areas")} style={{ background: "none", border: "none", cursor: "pointer", color: scrolled ? B.textMuted : "rgba(255,255,255,0.75)", fontSize: 14, fontFamily: "'Outfit',sans-serif", fontWeight: 500, padding: "6px 10px" }}>
              {lang === "en" ? "Areas" : "Áreas"}
            </button>

            <div style={{ width: 1, height: 24, background: scrolled ? "rgba(74,44,10,0.15)" : "rgba(255,255,255,0.2)", margin: "0 6px" }} />

            <button onClick={() => setLang(lang === "en" ? "es" : "en")} className="lang-btn"
              style={{ border: `1.5px solid ${scrolled ? B.greenDark : "rgba(255,255,255,0.35)"}`, color: scrolled ? B.greenDark : "white" }}>
              <Globe size={13} /> {lang === "en" ? "ES" : "EN"}
            </button>

            <a href="tel:6304497923" className="btn-cta" style={{ padding: "10px 20px", fontSize: 14 }}>
              <Phone size={13} /> (630) 449-7923
            </a>

            <button onClick={() => scrollTo("contact")} className="btn-green" style={{ padding: "10px 18px", fontSize: 14 }}>
              {t.nav.estimate}
            </button>
          </div>
          ...
          <div className="mob-only" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setLang(lang === "en" ? "es" : "en")} style={{ background: "none", border: "none", cursor: "pointer", color: scrolled ? B.greenDark : "white", padding: 4 }}>
              <Globe size={18} />
            </button>
            <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: "none", border: "none", cursor: "pointer", color: scrolled ? B.brown : "white", padding: 4 }}>
        ...
        {menuOpen && (
          <div style={{ background: B.offWhite, borderTop: `1px solid rgba(74,44,10,0.1)`, padding: "16px 24px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
            {[["services", lang === "en" ? "Services" : "Servicios"], ["testimonials", lang === "en" ? "Reviews" : "Reseñas"], ["areas", lang === "en" ? "Areas" : "Áreas"], ["contact", lang === "en" ? "Free Estimate" : "Presupuesto"]].map(([id, label]) => (
              <button key={id} onClick={() => scrollTo(id)} style={{ textAlign: "left", background: "none", border: "none", cursor: "pointer", color: B.textMuted, fontFamily: "'Outfit',sans-serif", fontWeight: 600, fontSize: 16, padding: "8px 0", borderBottom: `1px solid rgba(74,44,10,0.07)` }}>
                {label}
              </button>
            ))}
            <a href="tel:6304497923" className="btn-cta" style={{ padding: "13px 20px", fontSize: 15, justifyContent: "center", marginTop: 4 }}>
              <Phone size={15} /> (630) 449-7923
            </a>
          </div>
        )}
      </header>
      ...
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          <img src="/images/hero_background.jpg" alt="Hero Background" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
          {/* Brand-matched overlay: brown-to-green gradient */}
          ...
          <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(ellipse at 80% 20%, rgba(139,195,74,0.08) 0%, transparent 55%), radial-gradient(ellipse at 10% 80%, rgba(74,44,10,0.15) 0%, transparent 50%)" }} />
        </div>
        ...
        <div style={{ position: "relative", zIndex: 10, maxWidth: 1120, margin: "0 auto", padding: "130px 24px 90px", width: "100%" }}>
          ...
          <h1 className="h2 baskerville" style={{ fontSize: "clamp(58px, 10vw, 118px)", color: "white", lineHeight: 0.95, margin: "0 0 24px", maxWidth: 720 }}>
            {t.hero.h1a}<br />
            <em style={{ color: B.greenAccent, fontStyle: "italic" }}>{t.hero.h1b}</em>
          </h1>

          <p className="h3 outfit" style={{ color: "rgba(255,255,255,0.78)", fontSize: "clamp(16px, 2vw, 20px)", lineHeight: 1.55, marginBottom: 38, maxWidth: 520 }}>
            {t.hero.sub}
          </p>

          {/* CTA Row */}
          <div className="h4" style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 44 }}>
            <button onClick={() => scrollTo("contact")} className="btn-cta" style={{ padding: "15px 30px", fontSize: 16 }}>
              {t.hero.cta1} <ArrowRight size={17} />
            </button>
            <a href="tel:6304497923" className="btn-ghost" style={{ padding: "15px 28px", fontSize: 16 }}>
          ...
          <div className="h5" style={{ display: "flex", flexWrap: "wrap", gap: "10px 24px" }}>
            {[t.hero.t1, t.hero.t2, t.hero.t3, t.hero.t4].map((b, i) => (
              <span key={i} className="outfit" style={{ display: "flex", alignItems: "center", gap: 7, color: "rgba(255,255,255,0.7)", fontSize: 13, fontWeight: 500 }}>
                <CheckCircle size={13} color={B.greenAccent} /> {b}
              </span>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
            {t.services.items.map((svc, i) => {
              ...
              const Icon = icons[i];
              const images = [
                "/images/service_pruning.jpg",
                "/images/service_removal.jpg",
                "/images/service_clearing.jpg"
              ];
              return (
                <Fade key={i} delay={i * 90}>
                  <div className="svc-card" style={{ background: "white", borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 24px rgba(74,44,10,0.07)", border: `1px solid rgba(74,44,10,0.07)` }}>
                    <img src={images[i]} alt={svc.title} style={{ width: "100%", aspectRatio: "16/9", minHeight: 180, objectFit: "cover" }} />
                    <div style={{ padding: "22px 24px 26px" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "9px 0 10px" }}>
                        <div style={{ width: 34, height: 34, borderRadius: 8, background: B.greenPale, display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <Icon size={17} color={B.greenDark} />
                        </div>
      ...
      <section style={{ padding: "80px 24px", background: B.lightGray }}>
        <div style={{ maxWidth: 1120, margin: "0 auto" }}>
          <Fade>
            <h2 className="baskerville" style={{ fontSize: "clamp(26px, 4vw, 42px)", textAlign: "center", marginBottom: 50, color: B.brown }}>{t.why.title}</h2>
          </Fade>
            ...
            {t.why.items.map(({ icon: Icon, title, desc }, i) => (
              <Fade key={i} delay={i * 80}>
                <div style={{ background: "white", borderRadius: 14, padding: "26px", border: `1px solid rgba(74,44,10,0.08)`, display: "flex", gap: 16, alignItems: "flex-start" }}>
                  <div style={{ width: 42, height: 42, borderRadius: 10, background: B.greenPale, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon size={20} color={B.greenDark} />
                  </div>
                  <div>
      ...
      <section id="work" style={{ padding: "100px 24px", background: B.offWhite }}>
        <div style={{ maxWidth: 1120, margin: "0 auto" }}>
          ...
          {/* Mosaic grid */}
          <div style={{ display: "grid", gridTemplateColumns: "5fr 3fr 3fr", gridTemplateRows: "250px 250px", gap: 14 }}>
            {/* Big left */}
            <Fade style={{ gridColumn: "1", gridRow: "1 / 3" }}>
              <div style={{ gridColumn: "1", gridRow: "1 / 3", height: "100%" }}>
                <img src="/images/gallery_main.jpg" alt="Foto Estrella" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 16 }} />
              </div>
            </Fade>
            {/* Top mid */}
            <Fade delay={60} style={{ gridColumn: "2", gridRow: "1" }}>
              <div style={{ gridColumn: "2", gridRow: "1", height: "100%" }}>
                <img src="/images/gallery_before.jpg" alt="Antes" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14 }} />
              </div>
            </Fade>
            {/* Top right */}
            <Fade delay={100} style={{ gridColumn: "3", gridRow: "1" }}>
              <div style={{ gridColumn: "3", gridRow: "1", height: "100%" }}>
                <img src="/images/gallery_after.jpg" alt="Después" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14 }} />
              </div>
            </Fade>
            {/* Bot mid */}
            <Fade delay={140} style={{ gridColumn: "2", gridRow: "2" }}>
              <div style={{ gridColumn: "2", gridRow: "2", height: "100%" }}>
                <img src="/images/gallery_after.jpg" alt="Equipo Trabajando" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14 }} />
              </div>
            </Fade>
            {/* Bot right — video */}
            <Fade delay={180} style={{ gridColumn: "3", gridRow: "2" }}>
              <div style={{ gridColumn: "3", gridRow: "2", height: "100%" }}>
                <img src="/images/gallery_main.jpg" alt="Remoción en Acción" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14 }} />
              </div>
            </Fade>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto" }}>
          <Fade>
            <div style={{ textAlign: "center", marginBottom: 52 }}>
              <div style={{ display: "flex", justifyContent: "center", gap: 4, marginBottom: 14 }}>
                {[...Array(5)].map((_, i) => <Star key={i} size={20} fill={B.greenDark} color={B.greenDark} />)}
              </div>
              ...
              <Fade key={i} delay={i * 80}>
                <div style={{ background: "white", borderRadius: 16, padding: "26px", border: `1px solid rgba(74,44,10,0.07)`, position: "relative" }}>
                  ...
                  <div style={{ display: "flex", gap: 2, marginBottom: 14 }}>
                    {[...Array(5)].map((_, s) => <Star key={s} size={13} fill={B.greenDark} color={B.greenDark} />)}
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ width: 38, height: 38, borderRadius: "50%", background: B.greenPale, border: `1.5px dashed rgba(27,94,47,0.25)`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <Camera size={13} color={B.greenDark} opacity={0.5} />
                    </div>
                    <div>
          ...
          <Fade delay={180}>
            <div style={{ textAlign: "center", marginTop: 36 }}>
              <a href="#" style={{ display: "inline-flex", alignItems: "center", gap: 7, color: B.greenDark, fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: 14, textDecoration: "none", border: `1.5px solid ${B.greenDark}`, borderRadius: 6, padding: "10px 20px" }}>
                <Star size={13} /> {lang === "en" ? "See All Google Reviews" : "Ver Todas las Reseñas"} <ChevronRight size={13} />
              </a>
            </div>
          </Fade>
        </div>
          ...
          <Fade delay={80}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center" }}>
              {t.areas.cities.map((city, i) => (
                <span key={i} className="pill outfit" style={{ padding: "8px 18px", borderRadius: 100, border: `1.5px solid ${B.greenDark}`, color: B.greenDark, fontSize: 13, fontWeight: 600 }}>
                  {city}
                </span>
              ))}
              <span className="outfit" style={{ padding: "8px 18px", borderRadius: 100, background: B.lightGray, color: B.textLight, fontSize: 13, fontWeight: 500 }}>
                +15 {lang === "en" ? "more" : "más"}…
              </span>
            </div>
          </Fade>
      ...
      <section id="contact" style={{ padding: "100px 24px", background: B.brownDark, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 60, alignItems: "center" }}>
            ...
            <div>
              ...
              {submitted ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                  ...
                  <button type="submit" className="btn-cta" style={{ padding: "15px", fontSize: 16, justifyContent: "center", marginTop: 4, borderRadius: 8 }}>
                    {t.form.btn} <ArrowRight size={17} />
                  </button>
                  <p className="outfit" style={{ color: "rgba(255,255,255,0.4)", fontSize: 13, textAlign: "center", margin: 0 }}>
                    {t.form.or}{" "}
                    <a href="tel:6304497923" style={{ color: B.greenAccent, fontWeight: 700, textDecoration: "none" }}>(630) 449-7923</a>
                  </p>
                </form>
              )}
            </div>
          ...
          {/* RIGHT — Info + team photo */}
          <Fade delay={140}>
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>

              {/* LOGO on dark bg */}
              <div style={{ display: "flex", justifyContent: "center", padding: "20px 0 8px" }}>
                <Logo height={64} darkBg={true} />
              </div>

              {/* Team photo slot */}
              <img src="/images/team_photo.jpg" alt="Equipo JPC" style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", borderRadius: 14 }} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "12px 14px" }}>
                    ...
                    {href
                      ? <a href={href} className="outfit" style={{ color: "rgba(255,255,255,0.8)", fontSize: 13, fontWeight: 500, textDecoration: "none", lineHeight: 1.3 }}>{val}</a>
                      : <span className="outfit" style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, lineHeight: 1.3 }}>{val}</span>}
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ══════════════════════════════════════════
          FLOATING MOBILE CTA
      ══════════════════════════════════════════ */}
      <a href="tel:6304497923" className="btn-cta mob-only"
        style={{ position: "fixed", bottom: 20, left: "50%", transform: "translateX(-50%)", padding: "14px 30px", fontSize: 15, borderRadius: 100, zIndex: 200, boxShadow: "0 8px 32px rgba(74,44,10,0.4)", whiteSpace: "nowrap" }}>
